...
void generate_experimentations()
{
   ...
   for (int i = 1; i < tablet.m - 1; i++)
   {
      ...
      printf("(%d, %d)\nCPU: %.2lf, %.2lf, %.2lf\nMemory: %d %d %d\n\n", i, tablet.m - 1 - i, cpu_time_used_dynamic, cpu_time_used_dynamic_symetric, cpu_time_used_hashmap, dynamic_memory_used, dynamic_symetric_memory_used, hashmap_memory_used);
   }
   fclose(memory_file);
   fclose(time_file);
